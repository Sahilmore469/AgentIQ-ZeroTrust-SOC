...
def build_fact_iam(raw_iam_df=None, dim_master=None):
    """
    Builds the logical FACT_IAM fact table.
    Contains IAM telemetry with foreign keys: user_id, hostname_norm, session_id_norm.
    """
    ...
    # Backfill hostname from master if user is present
    if dim_master is not None:
        m_host_map = dim_master.dropna(subset=['hostname_norm']).set_index('user_id')['hostname_norm'].to_dict()
        df['hostname_norm'] = df['hostname_norm'].fillna(df['user_id'].map(m_host_map))
        
    df['session_id_norm'] = df['session_id'].apply(standardize_session_id)
    df['department'] = df['department'].apply(standardize_department)
    
    # Backfill department from master if unknown
    if dim_master is not None:
        m_dept_map = dim_master.set_index('user_id')['department'].to_dict()
        unknown_mask = df['department'].isin(['Unknown', 'unknown', None]) | df['department'].isna()
        df.loc[unknown_mask, 'department'] = df.loc[unknown_mask, 'user_id'].map(m_dept_map).fillna('Unknown')
...
def build_fact_endpoint(raw_alerts_df=None, dim_master=None):
    """
    ...
    # Backfill hostname from master if available
    if dim_master is not None:
        m_host_map = dim_master.dropna(subset=['hostname_norm']).set_index('user_id')['hostname_norm'].to_dict()
        df['hostname_norm'] = df['hostname_norm'].fillna(df['user_id'].map(m_host_map))
    ...
    if 'status' in df.columns:
        df['status_norm'] = df['status'].apply(normalize_endpoint_status)
    ...
    if 'resolved_timestamp' in df.columns:
        ...
        df['impossible_resolution_flag'] = (
            ...
            (df['resolved_timestamp'] < df['detected_timestamp'])
        )
    else:
        df['impossible_resolution_flag'] = False
...
def validate_data_model(dim_master, fact_iam, fact_endpoint, fact_firewall):
    ...
    diagnostics['iam_to_master'] = {
        ...
        'match_pct': round(float((matched_iam / len(fact_iam)) * 100), 2) if len(fact_iam) else 0.0,
        'null_keys': int(iam_uids.isna().sum()),
        'master_key_duplicates': int(dim_master['user_id'].duplicated().sum())
    }

    # 2. Endpoint -> Master on user_id & hostname_norm
    ...
    diagnostics['endpoint_to_master'] = {
        ...
        'match_user_pct': round(float((matched_ep_u / len(fact_endpoint)) * 100), 2) if len(fact_endpoint) else 0.0,
        'matched_host_rows': int(matched_ep_h),
        'match_host_pct': round(float((matched_ep_h / len(fact_endpoint)) * 100), 2) if len(fact_endpoint) else 0.0,
    ...
    diagnostics['firewall_to_master'] = {
        ...
        'match_pct': round(float((matched_fw_h / len(fact_firewall)) * 100), 2) if len(fact_firewall) else 0.0,
